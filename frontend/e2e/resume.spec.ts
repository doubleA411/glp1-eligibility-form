...
test('resume path - reload in screen 7 and check it lands in screen 7', async ({ page }) => {
  ...
  await expect(page.getByTestId('screen-prompt')).toContainText('Have you ever been diagnosed with diabetes?')
})